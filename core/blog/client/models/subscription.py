...
class SubscriptionManager(models.Manager):
    def is_subscribed(self, subscriber, writer):
        ...
        return self.filter(subscriber=subscriber, writer=writer, active=True).exists()

    def active_subscriptions(self, user):
        """Return all active subscriptions for a user."""
        return self.filter(subscriber=user, active=True)

    def subscribe(self, subscriber, writer):
        """Subscribe a user to a writer if not already subscribed."""
        subscription, created = self.get_or_create(subscriber=subscriber, writer=writer)
        if not created:
            subscription.active = True
            subscription.start_date = timezone.now()  # Reset start date for new subscription
            subscription.save()
        return subscription
...
class Subscription(models.Model):
    ...
    class Meta:
        unique_together = ('subscriber', 'writer')  # Prevent multiple subscriptions
